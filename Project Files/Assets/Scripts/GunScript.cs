...
public class GunScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // If we are an assault rifle, set base scale to be 1.25
        if (name.Contains("Assault Rifle"))
            baseScale *= 1.25f;
    ...
    public void DropGun()
    {
        ...
        // If owning game object is enemy hand, discard from enemy
        if (owningGameObject.tag == "EnemyHand")
        {
            EnemyAI tmpEnemy = transform.GetComponentInParent<EnemyAI>();
            tmpEnemy.currentlyAiming = false;
            tmpEnemy.gun = null;
            nPCGun = false;
        }
        // Else, if ninja player AI, discard from it
        else if (owningGameObject.tag == "PlayerAIHand")
        {
            PlayerAI tmpPlayer = transform.GetComponentInParent<PlayerAI>();
            tmpPlayer.currentlyAiming = false;
            tmpPlayer.gun = null;
            nPCGun = false;
        }
        // Else, discard from player
        else
        {
